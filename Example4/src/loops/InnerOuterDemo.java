...
class Statistics {
 ...
 void display() {
	  ...
	  //Here we define a decima format object for displaying numbers
    ...
    for(int i=0; i<values.length; i++)
     ...
     //Here we create an object of the inner class Indicators
      ...
      System.out.println("\nAverage: "+decimalFormt.format(indicators.getAverage()));
      System.out.println("Stdandar Deviation: "+decimalFormt.format(indicators.getStdDeviation()));
  }
...
//Here we define method getstdDev() for class Indicators
    ...
    //the enclosing class
     for(int i=0; i<values.length; i++) 
      ...
      std_dev = Math.sqrt(sum/values.length);
